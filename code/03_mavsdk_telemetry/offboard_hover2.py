...
async def wait_until_armed(drone: System) -> None:
    print("Waiting until armed=True...")
...
async def print_one_status(drone: System) -> None:
    armed_value = None
    flight_mode_value = None
    altitude_value = None
    ...
    async for flight_mode in drone.telemetry.flight_mode():
        flight_mode_value = flight_mode
        break

    async for position in drone.telemetry.position():
        altitude_value = position.relative_altitude_m
        break

    print("\n===== PX4 Status =====")
    print(f"Armed: {armed_value}")
    print(f"Flight mode: {flight_mode_value}")
    print(f"Relative altitude: {altitude_value:.2f} m")


async def wait_for_takeoff_altitude(drone: System) -> bool:
    ...
    async for position in drone.telemetry.position():
        ...
        if elapsed > MAX_TAKEOFF_WAIT_S:
            print("Timeout: drone did not reach takeoff altitude.")
            await print_one_status(drone)
            return False
...
async def main() -> None:
    ...
    try:
        ...
        takeoff_ok = await wait_for_takeoff_altitude(drone)

        if not takeoff_ok:
            print("\nTakeoff failed or drone did not climb.")
